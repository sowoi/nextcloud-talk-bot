...
class TestNextcloudUser(unittest.TestCase):
    def setUp(self):
        ...
        self.nextcloud_user = NextcloudUser(self.base_url, self.user, self.password)

    def test_test_user_login(self):
        mock_response = {"ocs": {"data": {"id": self.user}}}
        self.nextcloud_user.nextcloud_requests.send_request = MagicMock(return_value=mock_response)
