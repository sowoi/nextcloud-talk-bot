...
class TestNextcloudRequests(unittest.TestCase):
    ...
    def test_send_request(self):
        ...
        with unittest.mock.patch("requests.get", return_value=mock_response) as mock_get:
            ...
            mock_get.assert_called_with(
                f"{self.base_url}{endpoint}", headers=self.nextcloud_requests.headers, params=None
            )
    ...
    def test_post_request(self):
        ...
        with unittest.mock.patch("requests.post", return_value=mock_response) as mock_post:
            response = self.nextcloud_requests.post_request(endpoint, json=json_payload)
            self.assertEqual(response, {"user": "testuser"})
            mock_post.assert_called_with(
                f"{self.base_url}{endpoint}", headers=self.nextcloud_requests.headers, json=json_payload
            )
    ...
    def test_delete_request(self):
        ...
        with unittest.mock.patch("requests.delete", return_value=mock_response) as mock_delete:
            ...
            mock_delete.assert_called_with(f"{self.base_url}{endpoint}", headers=self.nextcloud_requests.headers)
